...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        # Ensure all metrics are numpy arrays before saving
        metrics = np.array([
            float(mae), 
            float(mse), 
            float(rmse), 
            ...
            float(mspe), 
            float(rse), 
            float(corr) if isinstance(corr, (int, float)) else corr[0]
        ])
    ...
    def predict(self, setting, load=False):
        ...
        # Before saving metrics, ensure all are scalar values
        np.save(folder_path + 'metrics.npy', np.array([
            ...
            float(mspe),
            float(rse),
            float(corr[0]) if isinstance(corr, np.ndarray) else float(corr)  # Ensure corr is scalar
        ]))
